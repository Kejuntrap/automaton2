...
public class Joinmaton {        //和集合演算のオートマトンを構築
    ...
    public static void main(String[] args){
        ...
        HashMap<String,State> jomaton = calcjoin(fa1,fa2);
        testOutput(jomaton);

    }
    private static HashMap<String,State> calcjoin(HashMap<String, State> fa1,HashMap<String, State> fa2){
        String second="";
        ArrayList<String> firstend = new ArrayList<String>();
        for(String f:fa2.keySet()){
            if(fa2.get(f).isStart){
                second = f;
                fa2.get(f).isStart=false;
                break;
            }
        }
        for(String fe:fa1.keySet()){
            if(fa1.get(fe).isAccepted){
                firstend.add(fe);
                fa1.get(fe).isAccepted=false;

                if(!fa1.get(fe).transition.containsKey("EPS")){
                    ArrayList<String> tmp = new ArrayList<String>();
                    tmp.add(second);
                    fa1.get(fe).transition.put("EPS",tmp);
                }else{
                    ArrayList<String> tmp = fa1.get(fe).transition.get("EPS");
                    tmp.add(second);
                    fa1.get(fe).transition.put("EPS", tmp);
                }
            }
        ...
        HashMap<String,State> joinmaton = new HashMap<String,State>();
        for(String ss : fa1.keySet()){
            joinmaton.put(ss,fa1.get(ss));
        }
        for(String ss : fa2.keySet()){
            joinmaton.put(ss,fa2.get(ss));
        }
        return joinmaton;
    }
